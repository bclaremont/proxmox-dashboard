...
- [x] **VM efficiency buckets** — classify every VM into Bully (resource hog), Constrained (hitting ceiling), Over-provisioned (large allocation, low usage), or Inactive (off 30+ days); colour-coded summary card on Overview, click-through to affected guest list
...
- [x] **Proactive health patrol** — runHealthPatrol() sweep (configurable interval, default 30min) across every connection for missed/failed backup jobs (reusing the Calendar tab's schedule parser), storage nearing capacity on every cluster (not just primary — closed a real gap there), and VM/CT restart loops; settings + toggle in Alert Settings modal
