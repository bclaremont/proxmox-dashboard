...
## Node update workflow (Updates view) — why there's no "Apply Updates" button
Proxmox's REST API does not expose applying package updates as a task — only `GET/POST /nodes/{node}/apt/update` (list pending / refresh the apt index) exist. There is no `apt/upgrade` endpoint. Proxmox's own web UI applies updates by opening a live shell (termproxy) to the node and running `apt` interactively — it isn't a REST call either. Don't try to "finish" this feature by finding a hidden apply-updates endpoint; there isn't one. The intended flow is Drain → Console (user runs `apt update && apt -y dist-upgrade` themselves) → Reboot → Migrate Back, and PCC deliberately never executes shell commands on a node — every other action in this app is a structured, typed REST call, and staying that way is a deliberate boundary, not an oversight.

Shared helpers worth reusing rather than reimplementing (all near the maintenance-mode helpers, `proxmox-dashboard.html`, search for `_pccMigrateGuestAwait`):
- `_pccWaitForTask(node, upid, connId, timeoutMs)` — polls a UPID's own `/tasks/{upid}/status` until it stops running. Use this instead of the old fire-and-forget-then-600ms-delay pattern.
- `_pccWaitForHAResource(sid, targetNode, connId, timeoutMs)` — HA migrate/relocate requests don't return a UPID (the CRM picks them up async); this polls `/cluster/ha/status/current` until the resource reports itself on the target node.
- `_pccMigrateGuestAwait(guest, targetNode, opts, haSids)` — the one function to call to move a single VM/CT: HA-aware (routes through `/cluster/ha/resources/{sid}/migrate|relocate` if the guest is HA-managed, otherwise the plain qemu/lxc migrate endpoint) and only resolves once the move is *confirmed* complete. `haSids` comes from `loadHAResourceSids(connId)`. Used by both the hardened `openDrainNodeModal` and `openMigrateBackModal` — use it for any future "move this VM and wait" need instead of a raw `migrate` POST.
- `_pccRebootNode(node, connId)` / `confirmRebootNode(node, connId)` — node-level reboot (`POST /nodes/{node}/status {command:"reboot"}`) is a real UPID-returning task, same shape as start/stop/migrate; wasn't wired up anywhere before this.
- Drain manifest (`_saveDrainManifest`/`_getDrainManifest`/`_clearDrainManifest`, localStorage key `pcc-drain-manifest`) — records which guests were moved off a node during a drain so `openMigrateBackModal` can bring them back later, even after a page reload. Keyed by `${connId}|${node}`, mirrors the existing `pve-maintenance-nodes` pattern (`_maintKey`/`getMaintenanceNodes`).
