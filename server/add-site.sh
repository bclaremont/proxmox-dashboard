...
echo "# Allow PCC hub to reach Proxmox API — only if Proxmox host firewall is DISABLED."
echo "# If Proxmox firewall is enabled, add this rule via Datacenter → Firewall instead."
echo "PostUp     = iptables -I INPUT -s 10.99.0.1/32 -p tcp --dport 8006 -j ACCEPT"
echo "PreDown    = iptables -D INPUT -s 10.99.0.1/32 -p tcp --dport 8006 -j ACCEPT"
echo ""
echo "[Peer]  # PCC hub"
