...
### Added
- **VM Efficiency Buckets** (Overview) — colour-coded summary card classifying every non-template VM/CT into at most one bucket: **Bully** (sustained CPU/RAM ≥80%), **Constrained** (pinned at ≥95% — hitting the ceiling, distinct from Bully), **Over-provisioned** (≥8GiB RAM or ≥200GiB disk allocated with <20% average usage), or **Inactive** (stopped with no `qmstart`/`vzstart` task in the last 30 days — one `/nodes/{node}/tasks?since=` call per node, not per VM). Bully/Constrained/Over-provisioned reuse the same `_vmHistory` rolling buffer the anomaly badge already populates (session-only, ~10min window) rather than adding new polling. Click a bucket card for the list of affected guests with a reason and a jump straight to VM/CT detail.
- **Proactive Health Patrol** (Alert Settings → new section) — periodic background sweep (default 30min, configurable) across every connected cluster for the class of problem a threshold-based alert can't catch on its own: things nobody set a rule for. Checks missed/failed backup jobs (reusing the Scheduled Tasks Calendar's schedule parser to compute each job's last expected run, then checking recent vzdump task history — node-granularity only, since vzdump tasks don't carry the backup-job id), storage nearing capacity on **every** connection (the existing threshold check only ever looked at the primary cluster's storage — extra clusters had no capacity alerting at all until now), and VM/CT restart loops (3+ start/stop/reboot ops on the same guest within a rolling window, both configurable). Also fixed `notifCooldowns` being in-memory only — a page reload used to silently reset every dedupe key, barely noticeable at the old fixed 5min cooldown but would have refired every multi-hour Patrol notice on reload; now persisted (7-day pruned) alongside existing notification state.
- **Scheduled Tasks Calendar** (Scheduled Operations → Calendar tab) — unifies PCC's own schedules with PVE backup jobs (every connection) and PBS backup jobs (if already connected) into one weekly grid, so clashing start times across the fleet are visible at a glance instead of buried across three separate views. Parses the systemd-calendar-style schedule strings PVE/PBS actually emit for simple recurring jobs (day list/range/wildcard + one or more `HH:MM` times); step syntax (`00/6:00`) is deliberately left unparsed rather than guessed at, with unparsed jobs listed in a fallback table below the grid so nothing is silently dropped. Clash detection is exact-time-match only — job durations vary too much to estimate, so anything softer would just be a guess dressed up as data.
...
- **`server/reset-2fa.js`** — break-glass recovery script for when 2FA locks out the only admin account and there's no other admin to use the in-app override. Run directly on the PCC server via SSH: `node reset-2fa.js --list` / `node reset-2fa.js <username>`. Documented in SETUP.md under Troubleshooting.
- **Per-category alert suppression duration** (Admin → Security) — dismissing a health warning used to only last until the browser reload (sessionStorage). Now each category (Node Offline, Storage, High CPU/Memory, Task Errors, Capacity Critical/Warning) has its own configurable suppression window, default 24h up to permanent — idea borrowed from ProxMenux Monitor. Dismissal still auto-clears sooner if the underlying condition itself changes.
- **Fail2Ban status card** (Admin → Security) — read-only view of jails and currently-banned IPs from the PCC server's own fail2ban, if configured. Deliberately read-only: `pcc.service` runs unprivileged with `NoNewPrivileges=yes`, so instead of granting the app any new privilege, a small root-owned systemd timer (`pcc-fail2ban-status.timer`, optional, documented in SETUP.md) dumps status to a JSON file the app just reads.
- **Semi-automated node update workflow** (Updates view) — per node: "Drain & Prepare" (hardened — now HA-aware and waits for each migration to actually confirm complete, instead of firing all migrate requests and moving on after a flat 600ms), "Console" (opens a shell to the node), "Reboot", and "Migrate Back" (restores guests that were drained off, using a manifest persisted in localStorage so it survives a page reload — a node update can take many minutes and nobody should have to keep a modal open the whole time). Deliberately does **not** apply the actual package upgrade — Proxmox's REST API has no endpoint for that (only `GET/POST .../apt/update`, list/refresh-index — the real upgrade is only ever done via a live shell, same as Proxmox's own web UI). Console + manual `apt update && apt -y dist-upgrade` is the intended step in between Drain and Reboot; automating that would mean PCC executing arbitrary shell commands on production hosts, which every other action in this app deliberately avoids.
- **Update alerting by real category, not fake severity** — checked the actual Proxmox source (`PVE::API2::APT`): the apt/update API has no security/severity field at all, ever. `Priority` is plain APT priority (`optional`/`important`/etc.), never `"security"` — so the dashboard's old "Security Updates" counter (and a matching one on the per-node Nodes-view badge) had been silently showing 0 or matching nothing, always. Replaced both with two categories we can actually verify from the package name: **Kernel** (`pve-kernel-*`/`proxmox-kernel-*`/`pve-firmware` — won't take effect without a reboot) and **Core Proxmox** (`pve-manager`, `pve-cluster`, `corosync`, `pve-ha-manager`, `qemu-server`, `pve-container`, `libpve-*` — directly affects cluster/VM management). Now surfaced in three places: the Updates view itself (stat cards + a Category column replacing the old fake security badge), the Overview health banner (new `updKernel`/`updCore` categories, suppressible per-category like every other health warning), and the email/webhook digest.
